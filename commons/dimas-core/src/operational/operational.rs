...
mod tests {
	...
	fn up_stepping() {
		...
		let sub_operational = TestOperational { value: 0, state: OperationState::Created, subs };

		let mut subs: Vec<Box<dyn Operational>> = Vec::new();
		subs.push(Box::new(sub_operational));

		let mut operational = TestOperational { value: 0, state: OperationState::Created, subs };

		assert!(operational.manage_operation_state(OperationState::Active).is_ok());
	...
	fn down_stepping() {
		let subs: Vec<Box<dyn Operational>> = Vec::new();
		let sub_operational = TestOperational { value: 0, state: OperationState::Active, subs };

		let mut subs: Vec<Box<dyn Operational>> = Vec::new();
		subs.push(Box::new(sub_operational));

		let mut operational = TestOperational { value: 15, state: OperationState::Active, subs };

		assert!(operational.manage_operation_state(OperationState::Created).is_ok());
	...
	fn no_stepping() {
		let subs: Vec<Box<dyn Operational>> = Vec::new();
		let sub_operational = TestOperational { value: 0, state: OperationState::Standby, subs };

		let mut subs: Vec<Box<dyn Operational>> = Vec::new();
		subs.push(Box::new(sub_operational));

		let mut operational = TestOperational { value: 7, state: OperationState::Standby, subs };

		assert!(operational.manage_operation_state(OperationState::Standby).is_ok());
